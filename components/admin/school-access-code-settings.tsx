...
import {
  rotateSchoolSignupAccessCode,
  setSchoolSignupAccessCode,
  setSchoolSignupAccessRequirement,
} from "@/lib/actions";
...
export function SchoolAccessCodeSettings({
  ...
  function changeRequirement(nextEnabled: boolean) {
    startTransition(async () => {
      const result = await setSchoolSignupAccessRequirement({ schoolId, enabled: nextEnabled });
      if (!result.success) {
        if (needsAdminReauthentication(result)) {
          beginAdminReauthentication();
          return;
        ...
        toast({
          title: "Could not update signup access",
          description: result.error,
          variant: "destructive",
        });
        return;
      ...
      toast({
        title: nextEnabled ? "Access code required" : "Access code turned off",
        description: nextEnabled
          ? `New ${schoolName} accounts must enter the current code.`
          : `New ${schoolName} accounts no longer need a code. Email-domain and verification rules still apply.`,
      });
    });
  }
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          Choose whether new password and Google accounts need a private code before joining {schoolName}.
          Email-domain and verification rules always still apply.
        </CardDescription>
      </CardHeader>
      <CardContent className="space-y-4">
        <div className="flex flex-col gap-3 rounded-xl border bg-muted/20 p-4 sm:flex-row sm:items-center sm:justify-between">
          <div>
            <Label htmlFor="require-school-access-code" className="font-semibold">
              Require a school access code
            </Label>
            <p className="mt-1 text-xs text-muted-foreground">
              Turn this off to let eligible, verified users join without a code.
            </p>
          </div>
          <label className="inline-flex cursor-pointer items-center gap-2 text-sm font-medium">
            <input
              id="require-school-access-code"
              type="checkbox"
              role="switch"
              checked={enabled}
              disabled={pending}
              onChange={(event) => changeRequirement(event.target.checked)}
              className="h-5 w-5 accent-storm-electric"
            />
            {enabled ? "Required" : "Off"}
          </label>
        </div>
        {!enabled && (
          <div className="rounded-lg border border-emerald-300 bg-emerald-50 p-3 text-sm text-emerald-950 dark:border-emerald-800 dark:bg-emerald-950/40 dark:text-emerald-100">
            Access-code checks are off. The saved code below is retained and will work again if you re-enable this setting.
          </div>
        )}
        {code ? (
        ...
        <div className="rounded-xl border bg-muted/20 p-4">
          ...
          <div className="mt-2 flex flex-col gap-2 sm:flex-row">
            ...
            <Button
              ...
              disabled={pending || !enabled || customCode.trim().toUpperCase() === code}
            >
        ...
        <div className="flex flex-wrap items-center justify-between gap-3">
          ...
          <Button type="button" variant="outline" onClick={rotate} disabled={pending || !enabled}>
